...
В модуле generators.py содержатся функции:
1. filter_by_currency() - принимает на вход список словарей, содержащих транзакции,
 и возвращает итератор, где валюта операции соответствует заданной;
2. transaction_descriptions() - оторый принимает список словарей с транзакциями и 
возвращает описание каждой операции по очереди.
3. card_number_generator() - который выдает номера банковских карт в формате 
XXXX XXXX XXXX XXXX.

В модуле bank_utils.py реализованы функции поиска транзакций по заданной строке в описании и подсчета количества транзакций,
относящихся к указанным категориям. Тестирование к модулю содержит: 
- проверку базавого поиска с учетом регистра,
- проверку точного совпадения фраз,
- обработку пустой поисковой строки,
- подсчет совпадений по категориям,
- нечувствительность к регистру,
- проверку случаев без совпадений и пустой список категорий

Функции модуля main.py осуществляют чтение транзакций из разного типа файлов (JSON, CSV, XLSX). 
Функция main в модуле, которая отвечает за основную логику проекта с пользователем и связывает функциональности между собой.
В модуле decorators.py добавлен декоратор log, который автоматически регистрирует детали
выполнения функций, такие как время вызова, имя функции, передаваемые аргументы, результат 
выполнения и информация об ошибках. Это позволяет обеспечить более глубокий контроль и анализ 
поведения программы в процессе ее выполнения.
Декоратор протестирован на успешное выполнение записи в файл, на запись ошибки в файл, на вывод
успешного выполнения в консоль, и на вывод ошибки в консоль.
