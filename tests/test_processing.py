...
from tests.conftest import data_base, data_base_date


def test_filter_by_state(data_base: list, state: str = "EXECUTED") -> None:
    # Тест на фильстрацию по 'EXECUTED'
    assert filter_by_state(data_base, "EXECUTED") == [
        {"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"},
        {"id": 939719570, "state": "EXECUTED", "date": "2018-06-30T02:08:58.425572"},
    ]
    # Тест на фильтрацию по 'CANCELED'
    assert filter_by_state(data_base, "CANCELED") == [
...
def test_filter_by_state_empty(data_base: list) -> None:
...
@pytest.mark.parametrize(
    "state, expected_ids",
    [("EXECUTED", [41428829, 939719570]), ("CANCELED", [594226727, 615064591]), ("UNKNOWN_STATE", [])],
)
def test_filter_by_state_parametrized(state: str, expected_ids: int, data_base: list) -> None:
    assert [item["id"] for item in filter_by_state(data_base, state)] == expected_ids


# Тест на сортировку по убыванию
def test_sort_by_date(data_base_date: list, data_base: list) -> None:
